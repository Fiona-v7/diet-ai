...
function EmotionSummary({ meals }: { meals: Meal[] }) {
  ...
  if (allRealHunger) {
    emoji = '⭐';
    insight = '这一天每一次进食都是因为身体真的需要。';
  } else if (realHunger >= total / 2) {
    emoji = '💚';
    insight = `这一天 ${realHunger} 次进食是因为真的饿了。`;
  } else if (emotionalEating > realHunger && emotionalEating >= total / 2) {
    emoji = '🧘';
    insight = `这一天有 ${emotionalEating} 次进食和情绪有关。`;
  } else if (social >= 2) {
    emoji = '🎉';
    insight = `这一天有 ${social} 次社交场合的进食。`;
  } else {
    emoji = '💪';
...
export default function HistoryPage({ meals, onClose }: HistoryPageProps) {
  ...
  const totalSodium = dateMeals.reduce((sum, m) => sum + (m.sodium || 0), 0);
      ...
      {dateMeals.length > 0 && (
        <div className="bg-white rounded-lg shadow-sm p-5 mb-6">
          ...
          <div className="grid grid-cols-5 gap-2 text-center">
            ...
            <div>
              <p className="text-2xl font-bold text-indigo-600">{totalSodium}</p>
              <p className="text-xs text-gray-500">钠mg</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className={`text-xs px-2 py-0.5 rounded-full flex-shrink-0 ${emotionColors[meal.emotion] || 'bg-gray-100 text-gray-600'}`}>
              {meal.emotion}
            </span>
          </div>
